...
function canMoveTo(grid: string[], r: number, c: number): boolean {
    return (
        r >= 0 &&
        r < grid.length &&
        c >= 0 &&
...
function followPath(grid: string[]): [string | null, string | null] {
    ...
    while (true) {
        ...
        if (currentCell === "+" || (currentCell >= "A" && currentCell <= "Z")) {
            let possibleDirections = ["up", "down", "left", "right"].filter(
                (d) => d !== direction
            );
